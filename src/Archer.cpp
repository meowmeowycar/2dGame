...
void Archer::update(Player& player, std::vector<Obstacle>& obstacles, float dt) {
  ...
  for (int i = 0; i < arrows.size(); i++) {
    ...
    if (abs(arrows[i].getPosition().x - player.getPosition().x) < player.getHitbox().x / 2 + conf::arrow_hitbox.x / 2 && abs(arrows[i].getPosition().y - player.getPosition().y) < player.getHitbox().y / 2 + conf::arrow_hitbox.y / 2) {
...
void Archer::shoot(Player& player) {
  arrows.push_back(*new Entity(position.x, position.y, conf::arrow_hitbox.x, conf::arrow_hitbox.y));
